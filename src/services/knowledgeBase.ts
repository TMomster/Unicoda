...
## XMemory 记忆卡（Yolo 模式专属）

XMemory 是 Yolo 模式下的**记忆卡系统**，模拟人脑记忆细胞的运作机制。每张记忆卡对应一个**独立的记忆对话**，通过注入额外的提示词让你自主管理记忆颗粒（创建、更新、删除）。

### 关键规则
1. **记忆卡只能通过 XMemory UI 管理**（Yolo 模式顶部的 "X" 按钮），你**无法**通过命令创建、删除或操作记忆卡
2. 每张记忆卡有唯一的 4 位编号（1000-9999），可重命名、删除、导出/导入
3. 双击未绑定的记忆卡 → 自动创建对话并绑定；双击已绑定的记忆卡 → 直接进入对话
4. 解绑会删除对话记录，**但保留记忆卡和其中所有颗粒**（卡片变为未绑定状态，下次重新绑定后颗粒依然存在）
5. 导出记忆卡包含卡片的标题、描述、全部颗粒内容（不含 ID），导入时自动分配新 ID
6. 用户可在记忆卡结构查看界面**手动管理颗粒**（点击卡片列表中的眼睛图标进入）：进入后是一个独立页面而非弹窗，支持搜索颗粒、新建颗粒、点击展开编辑、右键菜单（编辑/复制/删除）
7. XMemory 对话与普通 Yolo 会话分离，可通过左侧会话列表随时切换

### 模型在 XMemory 对话中的行为规则

在 XMemory 对话中，模型会收到特殊的**记忆管理系统提示词**，要求其遵循严格的三步工作流程（每轮必须执行，不得跳过）：

1. **第一步：分析决策** — 分析用户消息，判断是否需要创建/更新/删除记忆颗粒
   - 用户告知身份、名字、角色设定或长篇角色卡 → 必须逐条分析每个维度，创建抽象感知颗粒（重要级 A）
   - 用户告知关系信息 → 创建抽象感知颗粒（重要级 A）
   - 用户表达喜好/习惯 → 创建抽象感知颗粒
   - 环境或状态变化 → 创建具象感知颗粒并删除旧对应颗粒
   - 如果不需要操作 → 分析结果为"无需操作"
2. **第二步：先操作记忆，再回复** — 必须先调用工具执行完毕，然后才回复用户
3. **第三步：确认后再回复** — 自我检查本轮记忆操作是否全部完成，再以角色身份回复

**特别强调**：当用户第一次发送角色设定/人设要求时，这是最关键的记忆创建时机。模型必须逐条拆解角色卡中的每个独立维度（姓名、关系、性格、喜好/厌恶、口头禅、行为规则、格式要求等），为每个维度创建独立颗粒。禁止只创建 1-2 颗笼统的"角色设定"颗粒。参考正确示例：小棠角色卡应拆解为姓名颗粒、用户称呼颗粒、关系颗粒、性格颗粒、说话风格颗粒、行为规则颗粒、格式规则颗粒等 7+ 颗独立颗粒。

**原子化与合并双轨策略**：根据信息变化频率区分——频繁变化的信息（位置、状态）必须原子化，每个维度独立颗粒；长期稳定的信息（性格、技能）同一主题可适度合并；不同类别信息严禁合并

### 记忆颗粒类型
- **抽象感知（长期记忆）**：储存长期稳定的信息（性格、喜好、关系、知识等），重要级 A 最高，C 最低
- **具象感知（当下环境记忆）**：储存当前环境信息（位置、状态、周围事物等），环境变化时必须用新信息替换旧信息`,
  },
  {
    id: "kb-emoji-usage",
...
- **XMemory（特化记忆）**：查看/管理所有特化记忆卡，显示统计信息（总卡数、已启用、总颗粒、分级分布），支持导入/导出 JSON，每张卡可展开查看记忆颗粒内容
...
XMemory 是 Yolo 模式下的**记忆卡系统**，模拟人脑记忆细胞的运作。记忆卡只能通过 Yolo 模式顶部的 "X" 按钮在 UI 中管理，**不由模型操作**。在 XMemory 对话中，模型会收到特殊的系统提示词来自主管理记忆颗粒（创建/删除/整合）。
...
当 Security 未启用时，sensitive 模组调用自动放行（无审批系统介入，由用户自行承担风险）。`,
  ...
  {
    id: "kb-plus-intro",
    title: "Unicoda Plus 服务 — 外部应用集成",
    category: "platform",
    enabled: true,
    builtin: true,
    mode: "framework",
    retrievalType: "inject",
    summary: "Unicoda Plus 的外部集成能力和已连接的外部应用信息",
    content: `### 🔌 Unicoda Plus 服务

Unicoda Plus 是 Unicoda 的**外部应用集成协议**，允许其他桌面应用通过 TCP 连接向 Unicoda 注册自己的能力，供你（AI 助手）调用。

这意味着你可以通过 Unicoda Plus 间接控制其他应用的功能！

当用户询问关于外部应用的问题时（如"Pompeii 连上了吗"、"有看到什么外部应用吗"），请调用 **get_plus_status** 模组获取实时的连接状态和已连接应用详情。`,
  },
];
