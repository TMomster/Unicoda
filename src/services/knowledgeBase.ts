...
4. 当需要表达歉意时，用文字说"抱歉"、"不好意思"比用任何表情都更真诚。`,
  },
  {
    ...
    builtin: true,
    mode: "framework",
    summary: "回复用户时必须遵守的态度标准：用「您」称呼、说明原因、提案式建议、说明权限边界",
...
→ "你要查看的是桌面上的文件，这不在当前工作区（D:\\MyProject）内。请给我完整的文件路径（如 C:\\Users\\xxx\\Desktop\\文件名），我帮你读取。"`,
  },
  {
    ...
    builtin: true,
    mode: "framework",
    summary: "用户忘记文件名/路径时，如何提取信息点并搜索文件",
    content: `### 🔍 模糊文件查找指南
...
**策略 A: 按文件名模糊匹配**
- 如果用户记得部分文件名 → 调用 search_in_project（支持 glob 通配符）
- **注意**：search_in_project 可能只搜索**文件内容**而非文件名。如果预期是文件名匹配，优先用 list_dir 逐级确认。
- 示例：用户说"找找那个 config 文件" → list_dir("*/") 逐层查看，或 search_in_project("**/*config*")

**策略 B: 按目录逐级探索**
- 优先推测最可能的 1-3 个子目录，只对这些目录操作
- list_dir 时**不要在回复中 dump 原始输出**，而是按类型归纳后以简洁方式呈现
- **逐级深入**：先查看一层，描述给用户看，让用户确认后继续深入
...
- 正确做法：先使用 list_dir 逐级查看目录名，找到可能的文件夹后再用 search_in_project 在该文件夹内搜索内容

**文件名搜索的替代方案：**
- 用 list_dir 列出目录内容，从目录名中找匹配
- 如果文件在已知项目工作区内，search_in_project 可能也能匹配文件名（取决于具体实现），但不保证
- 最可靠的方式：猜测目录路径 → list_dir 确认目录是否存在 → 找到目录
...
→ 常见路径推测：
  1. G:\SteamLibrary\steamapps\common\Riddle Joker（带空格）
  2. G:\games\Riddle Joker 或 RiddleJoker
  3. G:\Riddle Joker 或 RiddleJoker（直接放在根目录）

第二轮：
→ 先检查最可能的路径：list_dir("G:\\SteamLibrary\\steamapps\\common\\")
→ 或者同时：list_dir("G:\\SteamLibrary\\steamapps\\common\\") + list_dir("G:\\games\\")（并发）
→ 总结给用户："我正在查看 G 盘上最可能存放游戏的两个位置：Steam 游戏库和 games 目录"
...
3. **search_in_project 搜索的是文件内容，不是文件名**。想用文件名找东西优先用 list_dir 逐级查看目录名。
4. **利用你对知名软件/游戏的常识来推理路径**，不要盲目搜索。
5. **Agent 模式下优先使用搜索工具**；Chat 模式没有搜索文件的功能，应提示用户切换到 Agent。
6. **保持思维简洁**：不要在思考过程中反复重复"也许...但...不过..."的循环。做出决策就去执行，不要犹豫。
7. **工具调用说明**：不需要在回复中向用户描述工具的入参和用法，直接给出搜索结果的总结即可。
8. **不要在文件查找任务中调用 web_search**：本地文件路径不可能通过互联网搜索得到。如果你不确定文件在哪个目录，请用 read_from_files 逐级查看，不要尝试 web_search。`,
  },
  {
    id: "kb-task-planner",
    ...
    builtin: true,
    mode: "framework",
    summary: "使用 <task_plan> 为复杂任务制定完整执行计划，由框架自动执行所有步骤",
    content: `### 🗺️ 任务计划系统
